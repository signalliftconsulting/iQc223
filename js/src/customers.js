...
function _renderCustomers() {
  ...
  // Sort
  list.sort((a,b) => {
    let av = sortKey==='_delta'?getDelta7d(a): sortKey==='_momentum'?getDelta7d(a): sortKey==='status'?(a.status||''): sortKey==='lifecycle'?(a.lifecycle||''): sortKey==='tags'?(a.tags||[]).join(', '): sortKey==='name'?a.name: sortKey==='manager'?(a.manager||'zzz'): sortKey==='profile'?(a.scoring_profile||'zzz'): sortKey==='score'?a.score: sortKey==='mrr'?a.mrr||0: sortKey==='arr'?(a.arr||(a.mrr*12)||0): sortKey==='since'?(a.since||'9999'): sortKey==='created'?(a.created||'9999'): sortKey==='days'?(a.days != null ? a.days : 999): sortKey==='nps'?(a.nps != null ? a.nps : -1): sortKey==='csat'?(a.csat != null ? a.csat : -1): sortKey==='logins'?(a.logins != null ? a.logins : -1): sortKey==='adoption'?(a.adoption != null ? a.adoption : -1): sortKey==='growth'?(a.growth||'zzz'): sortKey==='renewal'?a.renewal||99: sortKey==='next_touch'?(a.next_touch||'9999'): sortKey==='tickets'?(a.tickets!=null?a.tickets:-1):0;
    let bv = sortKey==='_delta'?getDelta7d(b): sortKey==='_momentum'?getDelta7d(b): sortKey==='status'?(b.status||''): sortKey==='lifecycle'?(b.lifecycle||''): sortKey==='tags'?(b.tags||[]).join(', '): sortKey==='name'?b.name: sortKey==='manager'?(b.manager||'zzz'): sortKey==='profile'?(b.scoring_profile||'zzz'): sortKey==='score'?b.score: sortKey==='mrr'?b.mrr||0: sortKey==='arr'?(b.arr||(b.mrr*12)||0): sortKey==='since'?(b.since||'9999'): sortKey==='created'?(b.created||'9999'): sortKey==='days'?(b.days != null ? b.days : 999): sortKey==='nps'?(b.nps != null ? b.nps : -1): sortKey==='csat'?(b.csat != null ? b.csat : -1): sortKey==='logins'?(b.logins != null ? b.logins : -1): sortKey==='adoption'?(b.adoption != null ? b.adoption : -1): sortKey==='growth'?(b.growth||'zzz'): sortKey==='renewal'?b.renewal||99: sortKey==='next_touch'?(b.next_touch||'9999'): sortKey==='tickets'?(b.tickets!=null?b.tickets:-1):0;
