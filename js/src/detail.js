...
function _downloadAIContent(elementId, title) {
  ...
  // Get customer name from detail modal
  var custName = '';
  var nameEl = document.querySelector('#detail-modal .modal-hd h2') || document.querySelector('#qbr-modal .modal-hd h2');
  if (nameEl) custName = nameEl.textContent.replace(/[^\w\s-]/g, '').trim();
  var dateStr = new Date().toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' });
  // Open print window with styled content for Save as PDF
  var w = window.open('', '_blank', 'width=800,height=600');
  w.document.write('<!DOCTYPE html><html><head><title>' + escHtml(title || 'iQcadence') + '</title>');
  // Pull computed CSS variables from the app for consistent styling
  var cs = getComputedStyle(document.documentElement);
  var teal = cs.getPropertyValue('--teal').trim() || '#0f766e';
  ...
  var purple = cs.getPropertyValue('--purple').trim() || '#7c3aed';
  w.document.write('<style>');
  w.document.write(':root{--teal:' + teal + ';--blue:' + blue + ';--red:' + red + ';--green:' + green + ';--amber:' + amber + ';--purple:' + purple + '}');
  w.document.write('body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;max-width:700px;margin:0 auto;padding:40px 30px;color:#1e293b;font-size:13px;line-height:1.7}');
  w.document.write('h1{font-size:20px;margin:0 0 4px;color:#0f172a}');
  w.document.write('.header{border-bottom:2px solid ' + teal + ';padding-bottom:14px;margin-bottom:24px}');
  w.document.write('.meta{font-size:11px;color:#64748b;margin-top:4px}');
  w.document.write('.rec-box{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px 16px;margin-bottom:10px}');
  w.document.write('.rec-box__title{font-weight:700;font-size:11px;text-transform:uppercase;letter-spacing:.04em;color:#64748b;margin-bottom:6px}');
  w.document.write('.ta-card{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;margin-bottom:8px}');
  w.document.write('span[style*="background:var(--blue)"],span[style*="background:' + blue + '"]{background:' + blue + '!important;color:#fff!important;padding:1px 5px;border-radius:3px;font-size:9px;font-weight:700}');
  w.document.write('span[style*="background:var(--teal)"],span[style*="background:' + teal + '"]{background:' + teal + '!important;color:#fff!important;padding:1px 5px;border-radius:3px;font-size:9px;font-weight:700}');
  w.document.write('span[style*="background:var(--purple)"],span[style*="background:' + purple + '"]{background:' + purple + '!important;color:#fff!important;padding:1px 5px;border-radius:3px;font-size:9px;font-weight:700}');
  w.document.write('@media print{body{padding:20px}}');
  w.document.write('</style></head><body>');
  w.document.write('<div class="header"><h1>' + escHtml(title || 'iQcadence Export') + '</h1>');
  if (custName) w.document.write('<div class="meta">' + escHtml(custName) + ' &middot; ' + dateStr + ' &middot; Generated by iQcadence</div>');
  else w.document.write('<div class="meta">' + dateStr + ' &middot; Generated by iQcadence</div>');
