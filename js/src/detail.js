...
function _renderAIInsightsHTML(data) {
  ...
  html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">' + appIcon('sparkle', 16) + ' <span style="font-weight:700;font-size:var(--fs-base)">Overview</span><span style="font-size:var(--fs-2xs);color:var(--muted);text-transform:uppercase;letter-spacing:.04em">AI-Powered</span></div>';
...
function _downloadAIContent(elementId, title) {
  ...
  var fileName = (custName ? custName.replace(/\s+/g, '-').toLowerCase() + '-' : '') + (title || 'export').replace(/\.[^.]+$/, '').replace(/\s+/g, '-').toLowerCase() + '.pdf';

  // Build a styled container for PDF rendering
  var container = document.createElement('div');
  container.style.cssText = 'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;max-width:700px;padding:30px;color:#1e293b;font-size:13px;line-height:1.7;background:#fff';

  // Header
  var header = document.createElement('div');
  header.style.cssText = 'border-bottom:2px solid #0f766e;padding-bottom:14px;margin-bottom:24px';
  header.innerHTML = '<h1 style="font-size:20px;margin:0 0 4px;color:#0f172a">' + escHtml(title || 'iQcadence Export') + '</h1>' +
    '<div style="font-size:11px;color:#64748b;margin-top:4px">' + (custName ? escHtml(custName) + ' &middot; ' : '') + dateStr + ' &middot; Generated by iQcadence</div>';
  container.appendChild(header);

  // Clone content, strip buttons
  var clone = el2.cloneNode(true);
  clone.querySelectorAll('button').forEach(function(b) { b.remove(); });
  // Resolve CSS variables to actual colors for the PDF renderer
  var cs = getComputedStyle(document.documentElement);
  var varsMap = { '--teal':'#0f766e', '--blue':'#3b82f6', '--red':'#dc2626', '--green':'#16a34a', '--amber':'#d97706', '--purple':'#7c3aed', '--text':'#1e293b', '--muted':'#64748b', '--border':'#e2e8f0', '--bg':'#f8fafc' };
  Object.keys(varsMap).forEach(function(v) { varsMap[v] = cs.getPropertyValue(v).trim() || varsMap[v]; });
  var cloneHTML = clone.innerHTML;
  Object.keys(varsMap).forEach(function(v) { cloneHTML = cloneHTML.split('var(' + v + ')').join(varsMap[v]); });
  var content = document.createElement('div');
  content.innerHTML = cloneHTML;
  container.appendChild(content);

  // Temporarily add to DOM for rendering (off-screen)
